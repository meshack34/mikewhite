    ...
    if value is None:
        try:
            ...
            raise MpesaConfigurationException(
                'Mpesa environment not configured properly - ' + key + ' not found')
...
def generate_access_token_request(consumer_key=None, consumer_secret=None):
    ...
    consumer_key = consumer_key if consumer_key is not None else mpesa_config(
        'MPESA_CONSUMER_KEY')
    consumer_secret = consumer_secret if consumer_secret is not None else mpesa_config(
...
def mpesa_access_token():
    ...
    if access_token == None:
        ...
        now = timezone.now()
        print(now)
        print("Access token created at", access_token.created_at)
        if minutes > 50:
...
def encrypt_security_credential(credential):
    ...
    certificate_path = os.path.join(
        settings.BASE_DIR, 'certs', certificate_name)
